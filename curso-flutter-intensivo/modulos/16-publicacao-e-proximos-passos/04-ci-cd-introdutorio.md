...
> **Módulo:** 16 - Publicação e próximos passos · **Tempo estimado:** 45 min · **Nível:** Avançado
...
- [Aula 3 — Versionamento e releases](03-versionamento-e-releases.md) — tags `v*` disparam o
  pipeline.
- [Módulo 14, aula 7 — Assinatura no Gradle](../14-build-android/07-assinatura-no-gradle.md) — o
  bloco que lê variáveis de ambiente.
- [Módulo 15, aula 8 — Build: IPA e archive](../15-build-ios/08-build-ipa-e-archive.md) — o roteiro
...
jobs:
  ...
  android:
    ...
    steps:
      ...
      # do build.gradle.kts (módulo 14, aula 7).
      - name: Build AAB
        env:
...
Write-Host '  Estes saem do Mac (módulo 15, aula 7):'
...
if ($erros -gt 0) {
    ...
    Write-Host '     Módulo 14, aula 6.' -ForegroundColor Gray
}
...
| Variáveis lidas pelo `signingConfigs` | O mesmo `build.gradle.kts` serve local e CI (módulo 14, aula 7). |
