# 01 — Plano de estudo: Ritmo A ativo (15 dias)

> Este arquivo define **quanto** estudar por dia, **o que** estudar em cada dia e **como saber**
> que você pode avançar. Leia [00-como-usar-o-curso.md](00-como-usar-o-curso.md) antes deste.

⬅️ Anterior: [00-como-usar-o-curso.md](00-como-usar-o-curso.md) ·
➡️ Próximo: [02-configuracao-do-ambiente.md](02-configuracao-do-ambiente.md)

---

## 1. Visão dos três ritmos

A carga total do curso é **120 horas**, sempre. Nenhum ritmo corta conteúdo obrigatório.
O que muda é a **densidade diária**:

| | **Ritmo A — ativo** | Ritmo B | Ritmo C |
|---|---|---|---|
| Duração | **15 dias** | 30 dias | 60 dias |
| Carga total | **120 h** | 120 h | 120 h |
| Horas/dia | **8 h** | 4 h | 2 h |
| Divisão diária | **4 h aplicadas ao trabalho + 4 h intermitentes** | 3 h conteúdo + 1 h prática | 1 h 30 conteúdo + 30 min prática |
| Exercícios obrigatórios | **todos** | todos | todos |
| Exercícios opcionais | **os marcados ⭐** | livre | pular |
| Revisão | **dias 5, 10, 15** | dias 7, 14, 21, 28 | semanal |

> ⚡ **Seu plano ativo é o Ritmo A — Muito intensivo (15 dias).** Toda referência a “Dia N”
> para o seu estudo aponta para a seção 2. Os ritmos B e C ficam documentados como alternativas
> caso a carga de 8 horas diárias deixe de caber na rotina.

### Como escolher, se ainda estiver em dúvida

| Escolha | Se... |
|---|---|
| Muito intensivo (15 d) | Você consegue reservar 8 h/dia e tem espaço para aplicar parte do estudo em trabalho real |
| **Intensivo recomendado (30 d)** | Você tem meio período livre e quer resultado rápido sem quebrar |
| Moderado (60 d) | Você trabalha ou estuda em período integral e tem 2 h por dia |

**Não** misture ritmos no meio do curso sem refazer a conta. Se precisar mudar, veja a
seção 7 ("Se você atrasar").

---

## 2. Ritmo A — Muito intensivo (15 dias · 8 h/dia)

| Item | Valor |
|---|---|
| **Duração total** | 15 dias corridos |
| **Carga horária total** | 120 horas |
| **Horas diárias** | 8 h |
| **Divisão diária** | 4 h aplicadas ao trabalho + 4 h intermitentes de estudo e prática |
| **Exercícios obrigatórios** | Todos (os marcados "Obrigatório? Sim") |
| **Exercícios opcionais** | Somente os marcados com ⭐ nas listas |
| **Momentos de revisão** | Dias 5, 10 e 15 |
| **Avaliações de módulo** | Todas as 17, no mesmo dia em que o módulo termina |
| **Avaliações cumulativas** | Dias 5, 9, 12, 13 e 15 |
| **Projetos** | Projeto 1 no dia 7 · Projeto 2 no dia 10 · Projeto Final no dia 14 |

### 2.1 Conteúdo por dia

| Dia | Conteúdo | Entrega do dia |
|---|---|---|
| 1 | [02-configuracao-do-ambiente.md](02-configuracao-do-ambiente.md) + [módulo 00](modulos/00-git-e-terminal/README.md) completo + [módulo 01](modulos/01-logica-e-fundamentos/README.md) aulas 1–5 | `flutter doctor` sem erro de SDK; repositório Git criado |
| 2 | [Módulo 01](modulos/01-logica-e-fundamentos/README.md) aulas 6–10 + [exercícios M01](exercicios/01-logica-e-fundamentos.md) + [avaliação](avaliacoes/modulo-01-logica-e-fundamentos.md) | Lista M01 concluída |
| 3 | [Módulo 02](modulos/02-dart-basico/README.md) completo + [exercícios M02](exercicios/02-dart-basico.md) + [avaliação](avaliacoes/modulo-02-dart-basico.md) | Programas Dart de terminal rodando |
| 4 | [Módulo 03](modulos/03-dart-intermediario/README.md) completo + [exercícios M03](exercicios/03-dart-intermediario.md) + [avaliação](avaliacoes/modulo-03-dart-intermediario.md) | Classes, herança e generics |
| 5 | **Revisão** + [módulo 04](modulos/04-dart-avancado/README.md) completo + [avaliação M04](avaliacoes/modulo-04-dart-avancado.md) + [cumulativa 01](avaliacoes/cumulativa-01-dart.md) | Cumulativa 01 aprovada |
| 6 | [Módulo 05](modulos/05-introducao-ao-flutter/README.md) completo + [exercícios M05](exercicios/05-introducao-ao-flutter.md) + [avaliação](avaliacoes/modulo-05-introducao-ao-flutter.md) | Primeiro app rodando |
| 7 | [Projeto 1](projetos/01-projeto-iniciante/README.md) + [módulo 06](modulos/06-widgets-e-layouts/README.md) aulas 1–6 | Projeto 1 funcionando |
| 8 | [Módulo 06](modulos/06-widgets-e-layouts/README.md) aulas 7–12 + [exercícios M06](exercicios/06-widgets-e-layouts.md) + [avaliação](avaliacoes/modulo-06-widgets-e-layouts.md) | Telas com layout responsivo |
| 9 | [Módulo 07](modulos/07-navegacao-e-formularios/README.md) completo + [avaliação](avaliacoes/modulo-07-navegacao-e-formularios.md) + [cumulativa 02](avaliacoes/cumulativa-02-flutter-ui.md) | Cumulativa 02 aprovada |
| 10 | **Revisão** + [Projeto 2](projetos/02-projeto-intermediario/README.md) completo | Projeto 2 funcionando |
| 11 | [Módulo 08](modulos/08-estado-e-arquitetura/README.md) completo + [avaliação](avaliacoes/modulo-08-estado-e-arquitetura.md) | App com Riverpod |
| 12 | [Módulo 09](modulos/09-consumo-de-api/README.md) + [módulo 10](modulos/10-persistencia-de-dados/README.md) + [cumulativa 03](avaliacoes/cumulativa-03-estado-e-dados.md) | API e banco local funcionando |
| 13 | [Módulo 11](modulos/11-recursos-nativos/README.md) + [módulo 12](modulos/12-testes-e-debug/README.md) + [módulo 13](modulos/13-desempenho-e-seguranca/README.md) + [cumulativa 04](avaliacoes/cumulativa-04-qualidade-e-plataforma.md) | Testes passando |
| 14 | [Projeto Final](projetos/03-projeto-final-multiplataforma/README.md) etapas 1–8 | App "Foco" completo |
| 15 | **Revisão** + [módulo 14](modulos/14-build-android/README.md) + [módulo 15](modulos/15-build-ios/README.md) + [módulo 16](modulos/16-publicacao-e-proximos-passos/README.md) + [cumulativa 05](avaliacoes/cumulativa-05-build-e-distribuicao.md) + [avaliação final](avaliacoes/avaliacao-final.md) | **APK + AAB assinados** |

### 2.2 Critérios objetivos para avançar de dia (ritmo A)

- [ ] Todos os exercícios obrigatórios do dia estão resolvidos e conferidos no gabarito.
- [ ] `flutter analyze` termina com **0 erros** no código do dia.
- [ ] A avaliação do módulo do dia teve **≥ 7 acertos em 10**.
- [ ] Você consegue explicar em voz alta o 📌 Resumo de cada aula do dia.

> ⚠️ **Aviso honesto sobre este ritmo.** Oito horas diárias de conteúdo novo têm alta taxa de
> abandono e esquecimento. O ritmo funciona melhor quando parte da prática resolve um problema
> real do trabalho, mas isso não substitui as pausas, o sono e os critérios de avanço.

### 2.3 Sua divisão diária: trabalho + prática intermitente

| Faixa | Duração | Uso no seu contexto |
|---|---:|---|
| Bloco aplicado | 4 h | Estudar e aplicar o conteúdo do dia em uma necessidade real do trabalho; registrar decisões, código e dúvidas |
| Blocos intermitentes | 4 h no total | Ler aulas, digitar exemplos, fazer exercícios obrigatórios, conferir o gabarito e executar a avaliação |

Não conte como estudo as horas em que apenas esteve disponível para o trabalho. Registre no
termômetro as horas em que houve leitura ativa, implementação, teste, revisão ou correção.
Se uma demanda urgente consumir o bloco aplicado, mova o conteúdo daquele dia inteiro para a
data seguinte: não pule exercícios nem avaliação para manter o número do calendário.

---

## 3. Ritmo C — Moderado (60 dias · 2 h/dia)

*(Apresentado antes do cronograma detalhado para você poder comparar; o ritmo principal está
na seção 4.)*

| Item | Valor |
|---|---|
| **Duração total** | 60 dias corridos |
| **Carga horária total** | 120 horas |
| **Horas diárias** | 2 h |
| **Divisão diária** | 1 h 30 de conteúdo + 30 min de prática |
| **Exercícios obrigatórios** | Todos |
| **Exercícios opcionais** | Pular (faça só se a avaliação do módulo apontar dificuldade) |
| **Momentos de revisão** | 1 revisão por semana (dias 7, 14, 21, 28, 35, 42, 49, 56) |
| **Avaliações de módulo** | Todas as 17 |
| **Avaliações cumulativas** | Dias 18, 30, 44, 48 e 60 |
| **Projetos** | Projeto 1 nos dias 20–21 · Projeto 2 nos dias 30–33 · Projeto Final nos dias 49–55 |

### 3.1 Como montar seus 60 dias

O plano de 60 dias é o plano de 30 dias **dividido ao meio**: cada dia do cronograma da
seção 4 vira dois dias aqui.

| Dia do plano de 30 | Vira, no plano de 60 | Como dividir |
|---|---|---|
| Dia 1 | Dias 1 e 2 | Dia 1: ambiente · Dia 2: módulo 00 |
| Dia 2 | Dias 3 e 4 | 2 ou 3 aulas por dia |
| Dia 3 | Dias 5 e 6 | Aulas em um dia, exercícios no outro |
| … | … | Sempre: conteúdo primeiro, exercícios depois |
| Dia 30 | Dias 59 e 60 | Dia 59: cumulativa 05 · Dia 60: avaliação final + checklists |

### 3.2 Conteúdo por semana (ritmo C)

| Semana | Dias | Conteúdo |
|---|---|---|
| 1 | 1–7 | [Ambiente](02-configuracao-do-ambiente.md) + [módulo 00](modulos/00-git-e-terminal/README.md) + [módulo 01](modulos/01-logica-e-fundamentos/README.md) aulas 1–6 |
| 2 | 8–14 | [Módulo 01](modulos/01-logica-e-fundamentos/README.md) aulas 7–10 + [exercícios](exercicios/01-logica-e-fundamentos.md) + [módulo 02](modulos/02-dart-basico/README.md) aulas 1–5 |
| 3 | 15–21 | [Módulo 02](modulos/02-dart-basico/README.md) aulas 6–10 + [exercícios](exercicios/02-dart-basico.md) + [módulo 03](modulos/03-dart-intermediario/README.md) aulas 1–6 |
| 4 | 22–28 | [Módulo 03](modulos/03-dart-intermediario/README.md) aulas 7–10 + [módulo 04](modulos/04-dart-avancado/README.md) + [cumulativa 01](avaliacoes/cumulativa-01-dart.md) |
| 5 | 29–35 | [Módulo 05](modulos/05-introducao-ao-flutter/README.md) + [Projeto 1](projetos/01-projeto-iniciante/README.md) |
| 6 | 36–42 | [Módulo 06](modulos/06-widgets-e-layouts/README.md) completo |
| 7 | 43–49 | [Módulo 07](modulos/07-navegacao-e-formularios/README.md) + [cumulativa 02](avaliacoes/cumulativa-02-flutter-ui.md) + [Projeto 2](projetos/02-projeto-intermediario/README.md) |
| 8 | 50–56 | [Módulo 08](modulos/08-estado-e-arquitetura/README.md) + [módulo 09](modulos/09-consumo-de-api/README.md) |
| 9 | 57–60 | [Módulo 10](modulos/10-persistencia-de-dados/README.md) a [módulo 16](modulos/16-publicacao-e-proximos-passos/README.md) + [Projeto Final](projetos/03-projeto-final-multiplataforma/README.md) + [avaliação final](avaliacoes/avaliacao-final.md) |

> No ritmo C a semana 9 concentra muito conteúdo. Se você escolher este ritmo, é normal
> estendê-lo para 70 dias. **Estender é melhor do que pular conteúdo obrigatório.**

### 3.3 Critérios objetivos para avançar (ritmo C)

- [ ] Exercícios obrigatórios do dia resolvidos.
- [ ] `flutter analyze` com 0 erros.
- [ ] Avaliação do módulo com **≥ 7 de 10**.
- [ ] Revisão semanal feita: reler os 📌 Resumos da semana e refazer 2 exercícios antigos.

---

## 4. Ritmo B — Intensivo de 30 dias (alternativa)

| Item | Valor |
|---|---|
| **Duração total** | 30 dias corridos |
| **Carga horária total** | 120 horas |
| **Horas diárias** | 4 h |
| **Divisão diária** | 3 h de conteúdo + 1 h de prática |
| **Exercícios obrigatórios** | Todos (mínimo de 8 por módulo) |
| **Exercícios opcionais** | Livres — faça os que couberem no tempo |
| **Momentos de revisão** | Dias **7, 14, 21 e 28** |
| **Avaliações de módulo** | As 17, ao final de cada módulo |
| **Avaliações cumulativas** | Dias 9, 15, 22, 24 e 30 |
| **Avaliação final** | Dia 30 |
| **Projetos** | Projeto 1 no dia 10 · Projeto 2 nos dias 15–16 · Projeto Final nos dias 25–27 |
| **Entregas principais** | Dia 10: app com estado · Dia 16: app com várias telas e persistência · Dia 27: app "Foco" completo · Dia 28: **APK + AAB assinados** · Dia 29: projeto iOS preparado · Dia 30: curso concluído |

### 4.1 Como é cada dia de 4 horas

| Bloco | Duração | O que fazer |
|---|---|---|
| 1 | 50 min | Aulas novas: ler 📖 Conceito, 💡 Analogia e 🧪 Exemplo mínimo, digitando os exemplos |
| pausa | 10 min | |
| 2 | 50 min | Aulas novas: 📱 Aplicando no Flutter, 💻 Código completo e 🔍 Explicando o código |
| pausa | 10 min | |
| 3 | 50 min | Últimas aulas do dia + 🛠️ Exercício guiado de cada uma |
| pausa | 10 min | |
| 4 | 50 min | 📝 Exercícios independentes obrigatórios + conferência no gabarito |
| fecho | 10 min | Reler os 📌 Resumos, marcar [03-trilha-de-progresso.md](03-trilha-de-progresso.md), commit do dia |

### 4.2 Visão geral dos 30 dias (alternativa)

| Dia | Conteúdo |
|---|---|
| 1 | [02-configuracao-do-ambiente.md](02-configuracao-do-ambiente.md) + módulo 00 |
| 2 | Módulo 01 (aulas 1–5) |
| 3 | Módulo 01 (aulas 6–10) + exercícios |
| 4 | Módulo 02 (aulas 1–5) |
| 5 | Módulo 02 (aulas 6–10) + exercícios |
| 6 | Módulo 03 (aulas 1–5) |
| 7 | **Revisão** + Módulo 03 (aulas 6–10) |
| 8 | Módulo 04 completo |
| 9 | Avaliação cumulativa 01 + Módulo 05 (aulas 1–5) |
| 10 | Módulo 05 (aulas 6–9) + **Projeto 1** |
| 11 | Módulo 06 (aulas 1–6) |
| 12 | Módulo 06 (aulas 7–12) |
| 13 | Módulo 07 (aulas 1–5) |
| 14 | **Revisão** + Módulo 07 (aulas 6–9) |
| 15 | Avaliação cumulativa 02 + **Projeto 2** (parte 1) |
| 16 | **Projeto 2** (parte 2) |
| 17 | Módulo 08 (aulas 1–5) |
| 18 | Módulo 08 (aulas 6–10) |
| 19 | Módulo 09 (aulas 1–5) |
| 20 | Módulo 09 (aulas 6–9) |
| 21 | **Revisão** + Módulo 10 |
| 22 | Avaliação cumulativa 03 + Módulo 11 |
| 23 | Módulo 12 |
| 24 | Módulo 13 + Avaliação cumulativa 04 |
| 25 | **Projeto Final** etapas 1–3 |
| 26 | **Projeto Final** etapas 4–5 |
| 27 | **Projeto Final** etapas 6–8 |
| 28 | **Revisão** + Módulo 14 (build Android) → **APK + AAB gerados** |
| 29 | Módulo 15 (build iOS) + Módulo 16 |
| 30 | Avaliação cumulativa 05 + Avaliação final + checklists |

---

## 5. Cronograma detalhado do Ritmo B (30 dias · alternativa)

### Dia 1 — Ambiente + Git e terminal

**Conteúdo (3 h)**
- [02-configuracao-do-ambiente.md](02-configuracao-do-ambiente.md) — resolver os três problemas desta máquina
- [00-git-e-terminal · README](modulos/00-git-e-terminal/README.md)
- [01 — O terminal sem medo](modulos/00-git-e-terminal/01-o-terminal-sem-medo.md)
- [02 — Arquivos e caminhos](modulos/00-git-e-terminal/02-arquivos-e-caminhos.md)
- [03 — Git: o que é](modulos/00-git-e-terminal/03-git-o-que-e.md)
- [04 — Commits, branches e .gitignore](modulos/00-git-e-terminal/04-commits-branches-gitignore.md)
- [05 — Desfazendo erros e segredos](modulos/00-git-e-terminal/05-desfazendo-erros-e-segredos.md)

**Prática (1 h)** — [exercícios M00](exercicios/00-git-e-terminal.md) obrigatórios ·
[gabarito](gabaritos/00-git-e-terminal.md) · [avaliação](avaliacoes/modulo-00-git-e-terminal.md) ·
[checklist ambiente Android](checklists/ambiente-android.md)

**Entrega:** Flutter SDK em caminho sem acento, Modo de Desenvolvedor ativo, Android SDK
instalado, `flutter doctor -v` sem erro de SDK, repositório Git criado com `.gitignore`.

### Dia 2 — Lógica e fundamentos (aulas 1–5)

**Conteúdo (3 h)**
- [01-logica-e-fundamentos · README](modulos/01-logica-e-fundamentos/README.md)
- [01 — O que é programar](modulos/01-logica-e-fundamentos/01-o-que-e-programar.md)
- [02 — Dart e Flutter](modulos/01-logica-e-fundamentos/02-dart-e-flutter.md)
- [03 — Algoritmos e decomposição](modulos/01-logica-e-fundamentos/03-algoritmos-e-decomposicao.md)
- [04 — Variáveis e constantes](modulos/01-logica-e-fundamentos/04-variaveis-e-constantes.md)
- [05 — Tipos de dados](modulos/01-logica-e-fundamentos/05-tipos-de-dados.md)

**Prática (1 h)** — 🛠️ exercícios guiados das 5 aulas + exercícios de fixação da
[lista M01](exercicios/01-logica-e-fundamentos.md).

### Dia 3 — Lógica e fundamentos (aulas 6–10) + exercícios

**Conteúdo (3 h)**
- [06 — Operadores](modulos/01-logica-e-fundamentos/06-operadores.md)
- [07 — Condições](modulos/01-logica-e-fundamentos/07-condicoes.md)
- [08 — Repetições](modulos/01-logica-e-fundamentos/08-repeticoes.md)
- [09 — Funções](modulos/01-logica-e-fundamentos/09-funcoes.md)
- [10 — Lendo mensagens de erro](modulos/01-logica-e-fundamentos/10-lendo-mensagens-de-erro.md)

**Prática (1 h)** — [lista M01 completa](exercicios/01-logica-e-fundamentos.md) ·
[gabarito](gabaritos/01-logica-e-fundamentos.md) ·
[avaliação do módulo 01](avaliacoes/modulo-01-logica-e-fundamentos.md)

### Dia 4 — Dart básico (aulas 1–5)

**Conteúdo (3 h)**
- [02-dart-basico · README](modulos/02-dart-basico/README.md)
- [01 — Anatomia de um programa](modulos/02-dart-basico/01-anatomia-de-um-programa.md)
- [02 — Sintaxe, convenções e comentários](modulos/02-dart-basico/02-sintaxe-convencoes-comentarios.md)
- [03 — var, final e const](modulos/02-dart-basico/03-var-final-const.md)
- [04 — Tipos, strings e conversões](modulos/02-dart-basico/04-tipos-strings-conversoes.md)
- [05 — Null safety](modulos/02-dart-basico/05-null-safety.md)

**Prática (1 h)** — exercícios de fixação e leitura de código da
[lista M02](exercicios/02-dart-basico.md).

### Dia 5 — Dart básico (aulas 6–10) + exercícios

**Conteúdo (3 h)**
- [06 — Controle de fluxo](modulos/02-dart-basico/06-controle-de-fluxo.md)
- [07 — Funções em Dart](modulos/02-dart-basico/07-funcoes-em-dart.md)
- [08 — Listas](modulos/02-dart-basico/08-listas.md)
- [09 — Sets e Maps](modulos/02-dart-basico/09-sets-e-maps.md)
- [10 — Entrada e saída](modulos/02-dart-basico/10-entrada-e-saida.md)

**Prática (1 h)** — [lista M02 completa](exercicios/02-dart-basico.md) ·
[gabarito](gabaritos/02-dart-basico.md) ·
[avaliação do módulo 02](avaliacoes/modulo-02-dart-basico.md)

### Dia 6 — Dart intermediário (aulas 1–5)

**Conteúdo (3 h)**
- [03-dart-intermediario · README](modulos/03-dart-intermediario/README.md)
- [01 — Classes e objetos](modulos/03-dart-intermediario/01-classes-e-objetos.md)
- [02 — Construtores](modulos/03-dart-intermediario/02-construtores.md)
- [03 — Encapsulamento](modulos/03-dart-intermediario/03-encapsulamento.md)
- [04 — Herança e polimorfismo](modulos/03-dart-intermediario/04-heranca-e-polimorfismo.md)
- [05 — Abstratas e interfaces](modulos/03-dart-intermediario/05-abstratas-e-interfaces.md)

**Prática (1 h)** — exercícios de aplicação da [lista M03](exercicios/03-dart-intermediario.md).

### Dia 7 — 🔁 REVISÃO + Dart intermediário (aulas 6–10)

**Revisão (1 h)** — releia os 📌 Resumos dos módulos
[00](modulos/00-git-e-terminal/README.md), [01](modulos/01-logica-e-fundamentos/README.md) e
[02](modulos/02-dart-basico/README.md); refaça 3 exercícios que você errou; confira o
[mapa de aprendizagem](04-mapa-de-aprendizagem.md).

**Conteúdo (2 h 30)**
- [06 — Mixins](modulos/03-dart-intermediario/06-mixins.md)
- [07 — Enums](modulos/03-dart-intermediario/07-enums.md)
- [08 — Generics](modulos/03-dart-intermediario/08-generics.md)
- [09 — Extensions](modulos/03-dart-intermediario/09-extensions.md)
- [10 — Arquivos, bibliotecas e pacotes](modulos/03-dart-intermediario/10-arquivos-bibliotecas-pacotes.md)

**Prática (30 min)** — [lista M03](exercicios/03-dart-intermediario.md) ·
[gabarito](gabaritos/03-dart-intermediario.md) ·
[avaliação do módulo 03](avaliacoes/modulo-03-dart-intermediario.md)

### Dia 8 — Dart avançado (módulo completo)

**Conteúdo (3 h)**
- [04-dart-avancado · README](modulos/04-dart-avancado/README.md)
- [01 — Exceptions](modulos/04-dart-avancado/01-exceptions.md)
- [02 — Futures e async/await](modulos/04-dart-avancado/02-futures-e-async-await.md)
- [03 — Streams](modulos/04-dart-avancado/03-streams.md)
- [04 — Records](modulos/04-dart-avancado/04-records.md)
- [05 — Patterns e switch](modulos/04-dart-avancado/05-patterns-e-switch.md)
- [06 — Sealed classes](modulos/04-dart-avancado/06-sealed-classes.md)
- [07 — Análise estática e lints](modulos/04-dart-avancado/07-analise-estatica-e-lints.md)
- [08 — Isolates e desempenho](modulos/04-dart-avancado/08-isolates-e-desempenho.md)

**Prática (1 h)** — [lista M04](exercicios/04-dart-avancado.md) ·
[gabarito](gabaritos/04-dart-avancado.md) ·
[avaliação do módulo 04](avaliacoes/modulo-04-dart-avancado.md)

### Dia 9 — Cumulativa 01 + Introdução ao Flutter (aulas 1–5)

**Avaliação (1 h)** — [Cumulativa 01 — Dart](avaliacoes/cumulativa-01-dart.md)
(cobre módulos 00 a 04) · respostas em
[gabarito-das-avaliacoes.md](gabaritos/avaliacoes.md)

**Conteúdo (2 h 30)**
- [05-introducao-ao-flutter · README](modulos/05-introducao-ao-flutter/README.md)
- [01 — Como o Flutter funciona](modulos/05-introducao-ao-flutter/01-como-o-flutter-funciona.md)
- [02 — Estrutura do projeto](modulos/05-introducao-ao-flutter/02-estrutura-do-projeto.md)
- [03 — main, runApp e a árvore de widgets](modulos/05-introducao-ao-flutter/03-main-runapp-arvore-de-widgets.md)
- [04 — StatelessWidget](modulos/05-introducao-ao-flutter/04-statelesswidget.md)
- [05 — StatefulWidget e setState](modulos/05-introducao-ao-flutter/05-statefulwidget-e-setstate.md)

**Prática (30 min)** — criar o projeto e rodar o app pela primeira vez.

> 🚦 **Critério de avanço obrigatório:** só siga para o dia 10 com **≥ 70 %** na cumulativa 01.
> Abaixo disso, use o dia 10 para refazer os módulos apontados na seção "Se você teve
> dificuldade" da própria avaliação.

### Dia 10 — Flutter (aulas 6–9) + PROJETO 1

**Conteúdo (2 h)**
- [06 — Ciclo de vida do State](modulos/05-introducao-ao-flutter/06-ciclo-de-vida-do-state.md)
- [07 — BuildContext](modulos/05-introducao-ao-flutter/07-buildcontext.md)
- [08 — Hot reload e hot restart](modulos/05-introducao-ao-flutter/08-hot-reload-e-hot-restart.md)
- [09 — Material e Cupertino](modulos/05-introducao-ao-flutter/09-material-e-cupertino.md)
- [lista M05](exercicios/05-introducao-ao-flutter.md) · [gabarito](gabaritos/05-introducao-ao-flutter.md) · [avaliação](avaliacoes/modulo-05-introducao-ao-flutter.md)

**Projeto 1 (2 h)** — "Meu Primeiro App"
- [README](projetos/01-projeto-iniciante/README.md)
- [01 — Especificação](projetos/01-projeto-iniciante/01-especificacao.md)
- [02 — Passo a passo](projetos/01-projeto-iniciante/02-passo-a-passo.md)
- [03 — Código completo](projetos/01-projeto-iniciante/03-codigo-completo.md)
- [04 — Testes](projetos/01-projeto-iniciante/04-testes.md)
- [06 — Checklist](projetos/01-projeto-iniciante/06-checklist.md)

**🎁 ENTREGA DO DIA 10:** app contador de sessões rodando, com tema claro/escuro.
Desafios opcionais em [05-desafios.md](projetos/01-projeto-iniciante/05-desafios.md)
(gabarito em [projeto-01-desafios.md](gabaritos/projeto-01-desafios.md)).

### Dia 11 — Widgets e layouts (aulas 1–6)

**Conteúdo (3 h)**
- [06-widgets-e-layouts · README](modulos/06-widgets-e-layouts/README.md)
- [01 — Scaffold e AppBar](modulos/06-widgets-e-layouts/01-scaffold-e-appbar.md)
- [02 — Texto, tipografia e ícones](modulos/06-widgets-e-layouts/02-texto-tipografia-icones.md)
- [03 — Container, Padding e SizedBox](modulos/06-widgets-e-layouts/03-container-padding-sizedbox.md)
- [04 — Row, Column e Expanded](modulos/06-widgets-e-layouts/04-row-column-expanded.md)
- [05 — Stack e Positioned](modulos/06-widgets-e-layouts/05-stack-e-positioned.md)
- [06 — Constraints](modulos/06-widgets-e-layouts/06-constraints.md)

**Prática (1 h)** — exercícios de aplicação da [lista M06](exercicios/06-widgets-e-layouts.md).

### Dia 12 — Widgets e layouts (aulas 7–12)

**Conteúdo (3 h)**
- [07 — Cores, temas e modo escuro](modulos/06-widgets-e-layouts/07-cores-temas-modo-escuro.md)
- [08 — Imagens e assets](modulos/06-widgets-e-layouts/08-imagens-e-assets.md)
- [09 — Listas e rolagem](modulos/06-widgets-e-layouts/09-listas-e-rolagem.md)
- [10 — Gestos e feedback](modulos/06-widgets-e-layouts/10-gestos-e-feedback.md)
- [11 — Responsividade](modulos/06-widgets-e-layouts/11-responsividade.md)
- [12 — Estados de UI](modulos/06-widgets-e-layouts/12-estados-de-ui.md)

**Prática (1 h)** — [lista M06 completa](exercicios/06-widgets-e-layouts.md) ·
[gabarito](gabaritos/06-widgets-e-layouts.md) ·
[avaliação do módulo 06](avaliacoes/modulo-06-widgets-e-layouts.md)

### Dia 13 — Navegação (aulas 1–5)

**Conteúdo (3 h)**
- [07-navegacao-e-formularios · README](modulos/07-navegacao-e-formularios/README.md)
- [01 — Navigator: a pilha](modulos/07-navegacao-e-formularios/01-navigator-a-pilha.md)
- [02 — Rotas nomeadas](modulos/07-navegacao-e-formularios/02-rotas-nomeadas.md)
- [03 — Argumentos e resultados](modulos/07-navegacao-e-formularios/03-argumentos-e-resultados.md)
- [04 — Abas e organização](modulos/07-navegacao-e-formularios/04-abas-e-organizacao.md)
- [05 — Navegação Android × iOS](modulos/07-navegacao-e-formularios/05-navegacao-android-x-ios.md) 🤖🍎

**Prática (1 h)** — exercícios de implementação da [lista M07](exercicios/07-navegacao-e-formularios.md).

### Dia 14 — 🔁 REVISÃO + Formulários (aulas 6–9)

**Revisão (1 h)** — releia os 📌 Resumos dos módulos
[05](modulos/05-introducao-ao-flutter/README.md) e [06](modulos/06-widgets-e-layouts/README.md);
refaça o [Projeto 1](projetos/01-projeto-iniciante/README.md) do zero, sem olhar o código pronto.

**Conteúdo (2 h 30)**
- [06 — Formulários](modulos/07-navegacao-e-formularios/06-formularios.md)
- [07 — Validação, foco e teclado](modulos/07-navegacao-e-formularios/07-validacao-foco-teclado.md)
- [08 — UX de formulários](modulos/07-navegacao-e-formularios/08-ux-de-formularios.md)
- [09 — go_router (opcional)](modulos/07-navegacao-e-formularios/09-go-router-opcional.md) — leitura opcional

**Prática (30 min)** — [lista M07](exercicios/07-navegacao-e-formularios.md) ·
[gabarito](gabaritos/07-navegacao-e-formularios.md) ·
[avaliação do módulo 07](avaliacoes/modulo-07-navegacao-e-formularios.md)

### Dia 15 — Cumulativa 02 + PROJETO 2 (parte 1)

**Avaliação (1 h)** — [Cumulativa 02 — Flutter UI](avaliacoes/cumulativa-02-flutter-ui.md)
(cobre módulos 05 a 07)

**Projeto 2 (3 h)** — "Bloco de Notas de Estudo"
- [README](projetos/02-projeto-intermediario/README.md)
- [01 — Especificação](projetos/02-projeto-intermediario/01-especificacao.md)
- [02 — Passo a passo](projetos/02-projeto-intermediario/02-passo-a-passo.md) (primeira metade: telas e navegação)

### Dia 16 — PROJETO 2 (parte 2)

**Projeto 2 (4 h)**
- [02 — Passo a passo](projetos/02-projeto-intermediario/02-passo-a-passo.md) (segunda metade: `Form`, validação e `shared_preferences`)
- [03 — Código completo](projetos/02-projeto-intermediario/03-codigo-completo.md)
- [04 — Testes](projetos/02-projeto-intermediario/04-testes.md)
- [06 — Checklist](projetos/02-projeto-intermediario/06-checklist.md)

**🎁 ENTREGA DO DIA 16:** app de notas com múltiplas telas, formulário validado e dados que
sobrevivem ao fechamento do app. Desafios em
[05-desafios.md](projetos/02-projeto-intermediario/05-desafios.md) ·
[gabarito](gabaritos/projeto-02-desafios.md).

### Dia 17 — Estado e arquitetura (aulas 1–5)

**Conteúdo (3 h)**
- [08-estado-e-arquitetura · README](modulos/08-estado-e-arquitetura/README.md)
- [01 — O problema do estado](modulos/08-estado-e-arquitetura/01-o-problema-do-estado.md)
- [02 — Elevação de estado](modulos/08-estado-e-arquitetura/02-elevacao-de-estado.md)
- [03 — InheritedWidget](modulos/08-estado-e-arquitetura/03-inheritedwidget.md)
- [04 — Por que Riverpod](modulos/08-estado-e-arquitetura/04-por-que-riverpod.md)
- [05 — Riverpod: primeiros passos](modulos/08-estado-e-arquitetura/05-riverpod-primeiros-passos.md)

**Prática (1 h)** — exercícios de fixação da [lista M08](exercicios/08-estado-e-arquitetura.md).

### Dia 18 — Estado e arquitetura (aulas 6–10)

**Conteúdo (3 h)**
- [06 — Notifier e NotifierProvider](modulos/08-estado-e-arquitetura/06-notifier-e-notifierprovider.md)
- [07 — AsyncNotifier e AsyncValue](modulos/08-estado-e-arquitetura/07-asyncnotifier-e-asyncvalue.md)
- [08 — family, autoDispose e listen](modulos/08-estado-e-arquitetura/08-family-autodispose-listen.md)
- [09 — Arquitetura feature-first](modulos/08-estado-e-arquitetura/09-arquitetura-feature-first.md)
- [10 — Injeção de dependências](modulos/08-estado-e-arquitetura/10-injecao-de-dependencias.md)

**Prática (1 h)** — [lista M08 completa](exercicios/08-estado-e-arquitetura.md) ·
[gabarito](gabaritos/08-estado-e-arquitetura.md) ·
[avaliação do módulo 08](avaliacoes/modulo-08-estado-e-arquitetura.md)

### Dia 19 — Consumo de API (aulas 1–5)

**Conteúdo (3 h)**
- [09-consumo-de-api · README](modulos/09-consumo-de-api/README.md)
- [01 — HTTP e REST](modulos/09-consumo-de-api/01-http-e-rest.md)
- [02 — JSON](modulos/09-consumo-de-api/02-json.md)
- [03 — Primeiro GET](modulos/09-consumo-de-api/03-primeiro-get.md)
- [04 — Modelando respostas e erros](modulos/09-consumo-de-api/04-modelando-respostas-e-erros.md)
- [05 — POST, PUT e DELETE](modulos/09-consumo-de-api/05-post-put-delete.md)

**Prática (1 h)** — exercícios de implementação da [lista M09](exercicios/09-consumo-de-api.md).

### Dia 20 — Consumo de API (aulas 6–9)

**Conteúdo (3 h)**
- [06 — Timeout, retry e cancelamento](modulos/09-consumo-de-api/06-timeout-retry-cancelamento.md)
- [07 — Camada de dados testável](modulos/09-consumo-de-api/07-camada-de-dados-testavel.md)
- [08 — Autenticação e tokens](modulos/09-consumo-de-api/08-autenticacao-e-tokens.md)
- [09 — API com Riverpod](modulos/09-consumo-de-api/09-api-com-riverpod.md)

**Prática (1 h)** — [lista M09 completa](exercicios/09-consumo-de-api.md) ·
[gabarito](gabaritos/09-consumo-de-api.md) ·
[avaliação do módulo 09](avaliacoes/modulo-09-consumo-de-api.md)

### Dia 21 — 🔁 REVISÃO + Persistência de dados (módulo completo)

**Revisão (45 min)** — releia os 📌 Resumos dos módulos
[08](modulos/08-estado-e-arquitetura/README.md) e [09](modulos/09-consumo-de-api/README.md);
reescreva do zero um `AsyncNotifier` que carrega dados de API.

**Conteúdo (2 h 45)**
- [10-persistencia-de-dados · README](modulos/10-persistencia-de-dados/README.md)
- [01 — Qual armazenamento usar](modulos/10-persistencia-de-dados/01-qual-armazenamento-usar.md)
- [02 — shared_preferences](modulos/10-persistencia-de-dados/02-shared-preferences.md)
- [03 — Arquivos e path_provider](modulos/10-persistencia-de-dados/03-arquivos-e-path-provider.md)
- [04 — sqflite: criando o banco](modulos/10-persistencia-de-dados/04-sqflite-criando-o-banco.md)
- [05 — sqflite: CRUD](modulos/10-persistencia-de-dados/05-sqflite-crud.md)
- [06 — Migrações](modulos/10-persistencia-de-dados/06-migracoes.md)
- [07 — Dados sensíveis](modulos/10-persistencia-de-dados/07-dados-sensiveis.md)
- [08 — Cache e offline](modulos/10-persistencia-de-dados/08-cache-e-offline.md)

**Prática (30 min)** — [lista M10](exercicios/10-persistencia-de-dados.md) ·
[gabarito](gabaritos/10-persistencia-de-dados.md) ·
[avaliação do módulo 10](avaliacoes/modulo-10-persistencia-de-dados.md)

### Dia 22 — Cumulativa 03 + Recursos nativos

**Avaliação (1 h)** — [Cumulativa 03 — Estado e dados](avaliacoes/cumulativa-03-estado-e-dados.md)
(cobre módulos 08 a 10)

**Conteúdo (2 h 30)**
- [11-recursos-nativos · README](modulos/11-recursos-nativos/README.md)
- [01 — Permissões](modulos/11-recursos-nativos/01-permissoes.md)
- [02 — Câmera e galeria](modulos/11-recursos-nativos/02-camera-e-galeria.md)
- [03 — Arquivos e compartilhamento](modulos/11-recursos-nativos/03-arquivos-e-compartilhamento.md)
- [04 — Notificações](modulos/11-recursos-nativos/04-notificacoes.md)
- [05 — Conectividade](modulos/11-recursos-nativos/05-conectividade.md)
- [06 — Ciclo de vida do app](modulos/11-recursos-nativos/06-ciclo-de-vida-do-app.md)
- [07 — Pastas android/ e ios/](modulos/11-recursos-nativos/07-pastas-android-e-ios.md) 🤖🍎
- [08 — Botão voltar e gestos](modulos/11-recursos-nativos/08-botao-voltar-e-gestos.md)
- [09 — Material × Cupertino](modulos/11-recursos-nativos/09-material-x-cupertino.md)
- [10 — Avaliando pacotes](modulos/11-recursos-nativos/10-avaliando-pacotes.md)

**Prática (30 min)** — [lista M11](exercicios/11-recursos-nativos.md) ·
[gabarito](gabaritos/11-recursos-nativos.md) ·
[avaliação do módulo 11](avaliacoes/modulo-11-recursos-nativos.md)

### Dia 23 — Testes e debug

**Conteúdo (3 h)**
- [12-testes-e-debug · README](modulos/12-testes-e-debug/README.md)
- [01 — Lendo stack traces](modulos/12-testes-e-debug/01-lendo-stack-traces.md)
- [02 — Logs e breakpoints](modulos/12-testes-e-debug/02-logs-e-breakpoints.md)
- [03 — DevTools](modulos/12-testes-e-debug/03-devtools.md)
- [04 — Análise, lint e formatação](modulos/12-testes-e-debug/04-analise-lint-formatacao.md)
- [05 — Testes unitários](modulos/12-testes-e-debug/05-testes-unitarios.md)
- [06 — Testes de widget](modulos/12-testes-e-debug/06-testes-de-widget.md)
- [07 — Mocks e fakes](modulos/12-testes-e-debug/07-mocks-e-fakes.md)
- [08 — Testes de integração](modulos/12-testes-e-debug/08-testes-de-integracao.md)
- [09 — Depurando Android e iOS](modulos/12-testes-e-debug/09-depurando-android-e-ios.md) 🤖🍎

**Prática (1 h)** — [lista M12](exercicios/12-testes-e-debug.md) ·
[gabarito](gabaritos/12-testes-e-debug.md) ·
[avaliação do módulo 12](avaliacoes/modulo-12-testes-e-debug.md)

**Entrega:** `flutter test` verde no Projeto 2.

### Dia 24 — Desempenho e segurança + Cumulativa 04

**Conteúdo (3 h)**
- [13-desempenho-e-seguranca · README](modulos/13-desempenho-e-seguranca/README.md)
- [01 — Rebuilds, const e keys](modulos/13-desempenho-e-seguranca/01-rebuilds-const-e-keys.md)
- [02 — Listas grandes e imagens](modulos/13-desempenho-e-seguranca/02-listas-grandes-e-imagens.md)
- [03 — Assíncrono sem travar](modulos/13-desempenho-e-seguranca/03-assincrono-sem-travar.md)
- [04 — Medindo desempenho](modulos/13-desempenho-e-seguranca/04-medindo-desempenho.md)
- [05 — Acessibilidade](modulos/13-desempenho-e-seguranca/05-acessibilidade.md)
- [06 — Segurança mobile](modulos/13-desempenho-e-seguranca/06-seguranca-mobile.md)
- [07 — Ofuscação e o que evitar](modulos/13-desempenho-e-seguranca/07-ofuscacao-e-o-que-evitar.md)
- [lista M13](exercicios/13-desempenho-e-seguranca.md) · [gabarito](gabaritos/13-desempenho-e-seguranca.md) · [avaliação](avaliacoes/modulo-13-desempenho-e-seguranca.md)

**Avaliação (1 h)** — [Cumulativa 04 — Qualidade e plataforma](avaliacoes/cumulativa-04-qualidade-e-plataforma.md)
(cobre módulos 11 a 13)

### Dia 25 — PROJETO FINAL: etapas 1 a 3

**Projeto (4 h)** — app "Foco", pacote `br.com.estudos.foco`
- [README](projetos/03-projeto-final-multiplataforma/README.md)
- [01 — Especificação](projetos/03-projeto-final-multiplataforma/01-especificacao.md)
- [02 — Arquitetura](projetos/03-projeto-final-multiplataforma/02-arquitetura.md)
- [03 — Etapa 1: Fundação](projetos/03-projeto-final-multiplataforma/03-etapa-1-fundacao.md)
- [04 — Etapa 2: Domínio e dados](projetos/03-projeto-final-multiplataforma/04-etapa-2-dominio-e-dados.md)
- [05 — Etapa 3: Estado com Riverpod](projetos/03-projeto-final-multiplataforma/05-etapa-3-estado-com-riverpod.md)

**Entrega do dia:** projeto criado, tema e rotas no lugar, banco `sqflite` criado, providers
respondendo.

### Dia 26 — PROJETO FINAL: etapas 4 e 5

**Projeto (4 h)**
- [06 — Etapa 4: Telas e navegação](projetos/03-projeto-final-multiplataforma/06-etapa-4-telas-e-navegacao.md)
- [07 — Etapa 5: API e trilhas](projetos/03-projeto-final-multiplataforma/07-etapa-5-api-e-trilhas.md)

**Entrega do dia:** as 5 telas navegáveis e a aba de trilhas carregando dados da API pública.

### Dia 27 — PROJETO FINAL: etapas 6 a 8

**Projeto (4 h)**
- [08 — Etapa 6: Responsividade e acessibilidade](projetos/03-projeto-final-multiplataforma/08-etapa-6-responsividade-e-acessibilidade.md)
- [09 — Etapa 7: Testes](projetos/03-projeto-final-multiplataforma/09-etapa-7-testes.md)
- [10 — Etapa 8: Ícone, splash e versão](projetos/03-projeto-final-multiplataforma/10-etapa-8-icone-splash-e-versao.md)
- [11 — Critérios de aceite](projetos/03-projeto-final-multiplataforma/11-criterios-de-aceite.md)
- [13 — Checklist](projetos/03-projeto-final-multiplataforma/13-checklist.md)
- [checklists/projeto-final.md](checklists/projeto-final.md)

**🎁 ENTREGA DO DIA 27:** app "Foco" completo, testado, com ícone e splash próprios.
Desafios opcionais em [12-desafios.md](projetos/03-projeto-final-multiplataforma/12-desafios.md) ·
[gabarito](gabaritos/projeto-03-desafios.md).

### Dia 28 — 🔁 REVISÃO + Build Android 🤖

**Revisão (45 min)** — rode o [checklist do projeto final](checklists/projeto-final.md)
inteiro e corrija o que estiver aberto.

**Conteúdo (2 h 15)**
- [14-build-android · README](modulos/14-build-android/README.md)
- [01 — Debug, profile e release](modulos/14-build-android/01-debug-profile-release.md)
- [02 — Identidade do app](modulos/14-build-android/02-identidade-do-app.md)
- [03 — Ícone](modulos/14-build-android/03-icone.md)
- [04 — Splash screen](modulos/14-build-android/04-splash-screen.md)
- [05 — Permissões no Android](modulos/14-build-android/05-permissoes-android.md)
- [06 — Keystore](modulos/14-build-android/06-keystore.md)
- [07 — Assinatura no Gradle](modulos/14-build-android/07-assinatura-no-gradle.md)
- [08 — Gerando APK e AAB](modulos/14-build-android/08-gerando-apk-e-aab.md)
- [09 — Instalando e validando](modulos/14-build-android/09-instalando-e-validando.md)
- [10 — Diagnóstico de build](modulos/14-build-android/10-diagnostico-de-build.md)

**Prática (1 h)** — [lista M14](exercicios/14-build-android.md) ·
[gabarito](gabaritos/14-build-android.md) ·
[avaliação do módulo 14](avaliacoes/modulo-14-build-android.md) ·
[checklist de build Android](checklists/build-android.md)

**🎁 ENTREGA DO DIA 28 — a mais importante do curso:**

| Artefato | Caminho gerado |
|---|---|
| APK release | `build/app/outputs/flutter-apk/app-release.apk` |
| APKs por ABI | `build/app/outputs/flutter-apk/app-arm64-v8a-release.apk` (+ `armeabi-v7a`, `x86_64`) |
| AAB | `build/app/outputs/bundle/release/app-release.aab` |

O APK precisa estar **instalado e funcionando em um celular Android de verdade** ao fim do dia.

### Dia 29 — Build iOS 🍎 + Publicação

**Conteúdo iOS (2 h)**
- [15-build-ios · README](modulos/15-build-ios/README.md)
- [01 — Por que exige macOS](modulos/15-build-ios/01-por-que-exige-macos.md)
- [02 — Xcode e CocoaPods](modulos/15-build-ios/02-xcode-e-cocoapods.md)
- [03 — Simulador e iPhone físico](modulos/15-build-ios/03-simulador-e-iphone-fisico.md)
- [04 — Bundle ID e Xcode](modulos/15-build-ios/04-bundle-id-e-xcode.md)
- [05 — Ícone, splash, versão e Info.plist](modulos/15-build-ios/05-icone-splash-versao-infoplist.md)
- [06 — Conta Apple gratuita × paga](modulos/15-build-ios/06-conta-apple-gratuita-x-paga.md)
- [07 — Certificados e provisioning](modulos/15-build-ios/07-certificados-e-provisioning.md)
- [08 — Build IPA e archive](modulos/15-build-ios/08-build-ipa-e-archive.md)
- [09 — Exportando IPA e TestFlight](modulos/15-build-ios/09-exportando-ipa-e-testflight.md)
- [10 — Diagnóstico de CocoaPods e assinatura](modulos/15-build-ios/10-diagnostico-cocoapods-e-assinatura.md)
- [lista M15](exercicios/15-build-ios.md) · [gabarito](gabaritos/15-build-ios.md) · [avaliação](avaliacoes/modulo-15-build-ios.md) · [checklist](checklists/build-ios.md) · [checklist de ambiente iOS](checklists/ambiente-ios.md)

> 🍎 **SÓ NO MAC.** Gerar o `.ipa`, abrir o Xcode e subir ao TestFlight exigem macOS + Xcode.
> No Windows você **lê, entende e deixa o projeto preparado** (Bundle ID, ícone, `Info.plist`,
> versões). Entenda o porquê técnico em
> [15-build-ios/01-por-que-exige-macos.md](modulos/15-build-ios/01-por-que-exige-macos.md).

**Conteúdo publicação (2 h)**
- [16-publicacao-e-proximos-passos · README](modulos/16-publicacao-e-proximos-passos/README.md)
- [01 — Google Play](modulos/16-publicacao-e-proximos-passos/01-google-play.md)
- [02 — App Store Connect](modulos/16-publicacao-e-proximos-passos/02-app-store-connect.md) 🍎
- [03 — Versionamento e releases](modulos/16-publicacao-e-proximos-passos/03-versionamento-e-releases.md)
- [04 — CI/CD introdutório](modulos/16-publicacao-e-proximos-passos/04-ci-cd-introdutorio.md)
- [05 — Monitoramento e feedback](modulos/16-publicacao-e-proximos-passos/05-monitoramento-e-feedback.md)
- [06 — Próximos passos](modulos/16-publicacao-e-proximos-passos/06-proximos-passos.md)
- [lista M16](exercicios/16-publicacao-e-proximos-passos.md) · [gabarito](gabaritos/16-publicacao-e-proximos-passos.md) · [avaliação](avaliacoes/modulo-16-publicacao-e-proximos-passos.md)

### Dia 30 — Cumulativa 05 + Avaliação final + checklists

**Avaliações (2 h 30)**
- [Cumulativa 05 — Build e distribuição](avaliacoes/cumulativa-05-build-e-distribuicao.md) (módulos 14 a 16)
- [Avaliação final](avaliacoes/avaliacao-final.md) (curso inteiro + projeto "Foco")
- Respostas: [gabarito-das-avaliacoes.md](gabaritos/avaliacoes.md)

**Fechamento (1 h 30)**
- [checklists/ambiente-android.md](checklists/ambiente-android.md)
- [checklists/ambiente-ios.md](checklists/ambiente-ios.md) 🍎
- [checklists/projeto-final.md](checklists/projeto-final.md)
- [checklists/build-android.md](checklists/build-android.md)
- [checklists/build-ios.md](checklists/build-ios.md) 🍎
- [03-trilha-de-progresso.md](03-trilha-de-progresso.md) — marcar tudo
- [referencias/proximos-passos.md](referencias/proximos-passos.md) — planejar o que vem depois

---

## 6. Critérios objetivos para avançar

### 6.1 Para avançar de **aula**

- [ ] Todas as caixas do ☑️ Checklist de domínio da aula estão marcadas.
- [ ] O 💻 Código completo da aula roda na sua máquina, digitado por você.
- [ ] Você resolveu o 🛠️ Exercício guiado sem olhar a resposta.

### 6.2 Para avançar de **módulo**

- [ ] Todas as aulas do módulo concluídas e marcadas em [03-trilha-de-progresso.md](03-trilha-de-progresso.md).
- [ ] **100 %** dos exercícios obrigatórios da lista do módulo resolvidos e conferidos.
- [ ] `flutter analyze` com **0 erros** e `dart format .` aplicado no código do módulo.
- [ ] Avaliação do módulo com **≥ 7 acertos em 10** questões.
- [ ] Exercício prático avaliativo da avaliação **rodando**, cumprindo os critérios de aceite.

### 6.3 Para avançar de **bloco** (avaliações cumulativas)

| Cumulativa | Dia no Ritmo A | Dia no Ritmo B | Cobre | Nota mínima | Se reprovar |
|---|---:|---:|---|---|---|
| [01 — Dart](avaliacoes/cumulativa-01-dart.md) | **5** | 9 | Módulos 00–04 | 70 % | Refaça módulos 02 e 03 antes de seguir |
| [02 — Flutter UI](avaliacoes/cumulativa-02-flutter-ui.md) | **9** | 15 | Módulos 05–07 | 70 % | Refaça o Projeto 1 do zero |
| [03 — Estado e dados](avaliacoes/cumulativa-03-estado-e-dados.md) | **12** | 22 | Módulos 08–10 | 70 % | Refaça o módulo 08 inteiro |
| [04 — Qualidade e plataforma](avaliacoes/cumulativa-04-qualidade-e-plataforma.md) | **13** | 24 | Módulos 11–13 | 70 % | Refaça os exercícios do módulo 12 |
| [05 — Build e distribuição](avaliacoes/cumulativa-05-build-e-distribuicao.md) | **15** | 30 | Módulos 14–16 | 70 % | Refaça o módulo 14 e gere o APK de novo |

### 6.4 Para considerar o **curso concluído**

- [ ] Os 17 módulos marcados na trilha.
- [ ] Os 3 projetos funcionando.
- [ ] APK **e** AAB de release assinados, gerados e instalados em um Android real.
- [ ] Projeto iOS preparado (Bundle ID `br.com.estudos.foco`, ícone, `Info.plist`, versões).
- [ ] 🍎 Se você teve acesso a um Mac: IPA gerado e versão no TestFlight.
- [ ] [Avaliação final](avaliacoes/avaliacao-final.md) com **≥ 70 %**.
- [ ] Os 5 arquivos de `checklists/` conferidos.

---

## 7. Se você atrasar

Atrasar é normal. Improvisar é o que estraga o plano. Use estas regras:

| Situação | O que fazer |
|---|---|
| Atrasou 1 dia | Não compense dobrando o dia seguinte. Empurre o cronograma em 1 dia |
| Atrasou 3 dias ou mais | Recalcule o Ritmo A a partir do dia pendente. Se 8 h/dia não forem mais sustentáveis, migre para o Ritmo B (30 d) ou C (60 d) antes de continuar |
| Faltou 1 semana inteira | Volte um módulo e refaça os exercícios antes de continuar |
| Está adiantado | **Não** pule dias de revisão. Use o tempo extra nos exercícios opcionais e nos 🏆 desafios |
| Travou num módulo específico | Aplique o método de depuração em 5 passos de [00-como-usar-o-curso.md](00-como-usar-o-curso.md) e consulte [referencias/erros-comuns.md](referencias/erros-comuns.md) |

**O que nunca cortar, em nenhum ritmo:**

1. Os dias de revisão (5, 10 e 15 no seu Ritmo A).
2. Os exercícios obrigatórios.
3. As 5 avaliações cumulativas e a final.
4. Os 3 projetos.
5. O dia 15 — é o dia em que o APK e o AAB nascem no seu plano.

---

## 8. O que você entrega ao longo do caminho

| Dia | Entrega verificável |
|---|---|
| 1 | `flutter doctor -v` sem erro de SDK; repositório Git com `.gitignore` correto |
| 3 | Programas Dart de terminal com condições, repetições e funções |
| 8 | Código Dart com `async`/`await`, `sealed class` e `switch` exaustivo |
| 10 | **Projeto 1** — contador de sessões com tema claro/escuro |
| 12 | Telas com layout responsivo e modo escuro |
| 16 | **Projeto 2** — notas com várias telas, formulário validado e persistência |
| 18 | App com Riverpod, `Notifier` e `AsyncNotifier` |
| 20 | App consumindo API pública com tratamento de erro e timeout |
| 21 | Banco `sqflite` com CRUD e migração |
| 23 | `flutter test` verde: unitários, widget e integração |
| 27 | **Projeto Final "Foco"** completo |
| 28 | **APK + AAB de release assinados e instalados** 🤖 |
| 29 | Projeto iOS preparado para build 🍎 |
| 30 | Avaliação final aprovada e checklists conferidos |

---

| ⬅️ Anterior | 🏠 Início | ➡️ Próximo |
|---|---|---|
| [00-como-usar-o-curso.md](00-como-usar-o-curso.md) | [README.md](README.md) | [02-configuracao-do-ambiente.md](02-configuracao-do-ambiente.md) |
